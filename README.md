...
- Replace `public/alex-morgan-cv.pdf` with your PDF, or use `/cv` → Print CV → Save as PDF after updating content. Set `profile.cvPdf` to the new path. The supplied PDF is a snapshot, not regenerated automatically when content changes.
